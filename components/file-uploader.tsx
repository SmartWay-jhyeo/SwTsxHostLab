...
export function FileUploader({ onDataLoaded, label, helpText, id }: FileUploaderProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col items-center justify-center border-2 border-dashed border-border rounded-lg p-6 text-center">
        ...
        <Button variant="outline" onClick={() => document.getElementById(id)?.click()} disabled={isLoading}>
          {isLoading ? "로딩 중..." : label}
        </Button>
        <input id={id} type="file" accept=".json" className="hidden" onChange={handleFileChange} disabled={isLoading} />
      </div>
    </div>
  )
}
